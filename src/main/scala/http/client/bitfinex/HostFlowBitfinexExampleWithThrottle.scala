...
/**
  ...
  *
  * This example also demonstrates how to use a throttle to control the rate of flow.
  */
object HostFlowBitfinexExampleWithThrottle extends App {
  ...
  val conn: Flow[(HttpRequest, String), (Try[HttpResponse], String), Http.HostConnectionPool] =
    Http().cachedHostConnectionPoolHttps[String]("api.bitfinex.com", 443)

  val symbols = List("btcusd", "ethusd", "omgusd", "xxxusd")  // xxxusd tests the non-200 Response Status case

  /**
    * Trottled source - emits 1 element downstream every 5 seconds
    * If you check the timestamp of the Responses in the logs you will see that they are
    * roughly 5 seconds apart.
    */
  val source = Source(symbols).throttle(1, 5.second, 1, ThrottleMode.shaping)
