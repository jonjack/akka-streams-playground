...
/**
  ...
  *
  * This example also demonstrates how to use an implicit Json marshaller to convert between a JSON
  * body and a Case Class.
  */
object HostFlowBitfinexExampleWithMarshalling extends App {
        ...
        case (map, (util.Success(resp), symbol)) => {
          resp.status.intValue match {
            ...
            case 200 => resp.entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
              ...
              // Marshalling from JSON body to Case Class representation
              import DefaultJsonProtocol._
              import StatsProtocol._
              val marshalledBody = body.utf8String.parseJson.convertTo[Array[Stat]]
              println("\r\nMarshalled Response for Request " + symbol +
                "\r\n--------------------------------------\r\nArray[Stat]\r\n\r\n" +
                marshalledBody.toJson.prettyPrint + "\r\n--------------------------------------\r\n")

              logsuccess.info("OK 200 [" + symbol + "] BODY[" + body.utf8String + "]")
            }
            case status => resp.entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
              logfailure.info("BAD " + status + " [" + symbol + "] BODY[" + body.utf8String + "]")
            }
          }
          map ++ Map(symbol -> resp)}
